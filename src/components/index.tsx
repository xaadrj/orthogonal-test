...
const ClockConstainer: React.FC<ClockConstainerProps> = () => {
  ...
  return (
    <div className=' relative flex justify-center items-center px-4 gap-10 flex-col w-full'>
      ...
      <div className={` ${Color === 'minus' ? 'bg-gray-100' : 'bg-gray-800' } ${Class} duration-500  ease-in-out transition-all shadow-lg rounded-lg p-1.5`}>
    <div className={`flex  ${Pos === 1 && "translate-x-48 duration-500" } ${Pos === 2 && "translate-y-40 translate-x-28 duration-700" } ${Pos === 3 && "translate-y-48 translate-x-0 duration-500" } ease-in-out  transition-all`}>
    <Clock Position={HandlePosition} Color={Color} />
      ...
      <div className='border-4 rounded-xl py-6 px-4  w-8/12'>
        ...
        <label htmlFor="mySelect" className='mr-4'>Theme:</label>
      <select className='border-2 focus:outline-none' id="mySelect" value={Color} onChange={handleColor}>
